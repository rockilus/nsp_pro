...
test.describe('Constraint Creation', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    const testRunId = `${testInfo.workerIndex}-${testInfo.title}-${randomUUID()}`;
    console.log(`[Test Run ${testRunId}] Starting constraint creation test setup`);
    ...
    await constraintTestBase.setupConstraintTests(testInfo.workerIndex, testRunId);
  ...
  test.afterEach(async ({}, testInfo) => {
    ...
    if (!constraintTestBase) {
      console.warn(`[Test Run ${testRunId}] No test base found - skipping cleanup`);
      return;
    }
    ...
    try {
      ...
      console.warn(`[Test Run ${testRunId}] Failed to cleanup test data:`, error);
    }
  ...
  test('should open constraint creation popup when clicking add constraint button', async ({
    ...
    const dialogTitle = page.locator('[data-testid="new-constraint-dialog-title"]');
    ...
    const placeholder = page.locator('[data-testid="select-template-placeholder"]');
  ...
  test('should show validation errors for incomplete constraint fields', async ({
    ...
    if (hasErrors) {
      const errorElements = page.locator('[data-testid*="constraint-block-error"], .Mui-error');
      ...
      console.log(`✅ Validation errors displayed for ${errorCount} incomplete fields`);
    } else {
      ...
      if (placeholderCount > 0) {
        ...
        console.log('✅ Validation error styling applied to incomplete placeholders');
      }
    }
  ...
  test('should close popup and add constraint to list when all fields are filled', async ({
    ...
    const selectedTemplate = page.locator('[data-testid^="template-item-"].Mui-selected');
    ...
    const workerBlock = page.locator('[data-testid^="shift-worker-option-block-4-"]');
    ...
    const anyWorkerOption = page.locator('[data-testid^="swo-option-Workers-"]');
    await expect(anyWorkerOption.first()).toBeVisible({ timeout: 10000 });

    // Now look for our specific worker
    const workerOption = page.locator(`[data-testid="swo-option-Workers-${testWorker.id}-false"]`);
    ...
    const shiftBlock = page.locator('[data-testid^="shift-worker-option-block-3-"]');
    await expect(shiftBlock.first()).toBeVisible();
    await shiftBlock.first().click();

    const shiftOption = page.locator(`[data-testid="swo-option-Shifts-${testShift.id}-false"]`);
    ...
    const validationErrors = page.locator('[data-testid*="constraint-block-error"], .Mui-error');
    ...
    console.log(`[Test ${testRunId}] ✅ Constraint creation test completed successfully!`);
  });
    ...
    for (let i = 0; i < placeholderCount; i++) {
      ...
      try {
        ...
        if (await shiftWorkerDialog.isVisible({ timeout: 2000 })) {
          ...
          // Confirm selection
          const confirmButton = constraintTestBase.getConfirmSelectionButton(page);
          ...
          if (await confirmButton.isVisible({ timeout: 1000 })) {
            ...
            const altConfirmButton = page
              .locator('button:has-text("OK"), button:has-text("Apply"), button:has-text("Save")')
              .first();
        ...
        console.log(`Could not interact with placeholder ${i}:`, (error as Error).message);
      }
    }
  ...
  test('should support multiple worker and shift selection', async ({ page }, testInfo) => {
    const testRunId = (testInfo as any).testRunId as string;
    const constraintTestBase = testBasesMap.get(testRunId)!;
      ...
      try {
        ...
        if (await shiftWorkerDialog.isVisible({ timeout: 2000 })) {
          ...
          if (await confirmButton.isVisible({ timeout: 1000 })) {
            ...
            if (await altConfirmButton.isVisible({ timeout: 1000 })) {
              await altConfirmButton.click();
              console.log('✅ Confirmed multiple selections with alternative button');
            }
          }
  ...
  test('should handle shift block interaction: display placeholder, show options, select shift, and validate', async ({
    ...
    const shiftBlocks = page.locator('[data-testid^="shift-worker-option-block-3-"]');
    ...
    const placeholder = shiftBlock.locator('[data-testid^="constraint-block-placeholder-"]');
    await expect(placeholder).toBeVisible();
    ...
    console.log(`Looking for test shift: ${testShift1.name} (ID: ${testShift1.id})`);
    ...
    console.log('Step 6: Verifying shift block displays selected shift name...');
    ...
    if (errorCount > 0) {
      ...
      console.log(`Shift block placeholder/name error indicators: ${placeholderHasError}`);

      if (shiftBlockHasError === 0 && placeholderHasError === 0) {
        console.log('✅ Shift block does NOT have validation errors (as expected)');
      } else {
  ...
  test('should handle worker block interaction: display placeholder, show options, select worker, and validate', async ({
    ...
    // The data-testid follows pattern: shift-worker-option-block-{blockName}-{index}
    const workerBlocks = page.locator('[data-testid^="shift-worker-option-block-4-"]');
    ...
    console.log('Step 1: Checking placeholder display...');
    const placeholder = workerBlock.locator('[data-testid^="constraint-block-placeholder-"]');
    await expect(placeholder).toBeVisible();
    const placeholderText = await placeholder.textContent();
    console.log(`✅ Placeholder displayed: "${placeholderText}"`);
    ...
    console.log(`Looking for test worker: ${testWorker1.name} (ID: ${testWorker1.id})`);
    ...
    console.log('Step 6: Verifying worker block displays selected worker name...');
    ...
    if (errorCount > 0) {
      ...
      console.log(`Worker block placeholder/name error indicators: ${placeholderHasError}`);

      if (workerBlockHasError === 0 && placeholderHasError === 0) {
        console.log('✅ Worker block does NOT have validation errors (as expected)');
      } else {
  ...
  test('should handle string block interaction: display placeholder, show options, select option, and validate', async ({
    ...
    const blockDisplay = page.locator('[data-testid="constraint-block-display-2"]');
    ...
    console.log('Step 5: The string block should now display the newly selected option');
    ...
    if (errorCount > 0) {
      ...
      const operatorBlockError = operatorBlock.first().locator('[data-testid*="error"]');
      ...
      console.log(`Operator block placeholder/name error indicators: ${placeholderHasError}`);

      if (operatorBlockHasError === 0 && placeholderHasError === 0) {
        console.log('✅ Operator block does NOT have validation errors (as expected)');
      } else {
  ...
  test('should handle number block interaction: display placeholder, enter number, and validate', async ({
    ...
    const placeholder = page.locator('[data-testid="constraint-block-placeholder-3"]');
    await expect(placeholder).toBeVisible();
    const placeholderText = await placeholder.textContent();
    console.log(`✅ Placeholder displayed: "${placeholderText}"`);

    console.log('Step 2: Click on the NUMBER block to open the text field input');
    await numberBlock.first().click();
    ...
    const blockDisplay = page.locator('[data-testid="constraint-block-display-3"]');
    await expect(blockDisplay).toBeVisible();
    ...
    if (errorCount > 0) {
      ...
      const numberBlockError = numberBlock.first().locator('[data-testid*="error"]');
      ...
      console.log(`Number block placeholder/name error indicators: ${placeholderHasError}`);

      if (numberBlockHasError === 0 && placeholderHasError === 0) {
        console.log('✅ Number block does NOT have validation errors (as expected)');
      } else {
