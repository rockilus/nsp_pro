...
test.describe('Constraint API Tests', () => {
  ...
  test('should create constraint via API successfully', async () => {
    ...
    if (templates.length === 0) {
      test.skip(true, 'No constraint templates available - cannot test constraint creation');
      return;
    }

    const template = templates[0];
    console.log(`Using template: ${template.id} (type: ${template.constraintType})`);
    ...
    const createdConstraint = await constraintTestBase.createTestConstraintFromTemplate(template, {
    ...
    const foundConstraint = constraints.find((c) => c.id === createdConstraint.id);
    ...
    console.log(`✅ Constraint verified in database with generated text: ${foundConstraint.text}`);
  ...
  test('should update constraint via API successfully', async () => {
    ...
    if (templates.length === 0) {
      test.skip(true, 'No constraint templates available - cannot test constraint update');
      return;
    }
    ...
    console.log(`✅ Constraint updated successfully: hard: ${foundConstraint.hard}`);
  ...
  test('should delete constraint via API successfully', async () => {
    ...
    if (templates.length === 0) {
      test.skip(true, 'No constraint templates available - cannot test constraint deletion');
      return;
    }
    ...
    let foundConstraint = constraints.find((c) => c.id === createdConstraint.id);
    expect(foundConstraint).toBeTruthy();
  ...
  test('should retrieve constraint templates successfully', async () => {
    ...
    if (templates.length > 0) {
      ...
      console.log(`✅ Template validation passed: ${template.id} - ${template.text}`);
    }
  });
  ...
  test('should create constraints with specific worker ID', async () => {
    ...
    const constraint1 = await constraintTestBase.createTestConstraintFromTemplate(template, {
    ...
    const constraint2 = await constraintTestBase.createTestConstraintFromTemplate(template, {
  ...
  test('should create constraints with all workers option', async () => {
    ...
    const constraint = await constraintTestBase.createTestConstraintFromTemplate(template, {
      workerId: 'all',
      shiftId: testShifts[0].id,
      numberValue: 3,
    });
  ...
  test('should create constraints with default parameters', async () => {
    ...
    expect(constraint.id).toBeTruthy();
    console.log(`✅ Created constraint with default parameters: ${constraint.id}`);
  });
  ...
  test('should create constraints with all combinations', async () => {
    ...
    const constraint3 = await constraintTestBase.createTestConstraintFromTemplate(template, {
      workerId: 'all',
      shiftId: 'no-duty',
      numberValue: 1,
    });
