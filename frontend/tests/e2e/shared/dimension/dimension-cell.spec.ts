...
test.describe('DimensionCell Component', () => {
  ...
  test.describe('Basic Popup Functionality', () => {
    ...
    test.beforeEach(async ({ page }) => {
      ...
      const dimensionName = `Test Dimension ${test.info().workerIndex}-${Date.now()}`;
      ...
      console.log(`Created test dimension: ${testDimension.name} (${testDimension.id})`);
    ...
    test.afterEach(async () => {
      ...
      if (testDimension?.id) {
        try {
          ...
          console.warn(`Failed to delete test dimension ${testDimension.id}:`, error);
        }
      }
    });

    test('should open popup when clicking on the dimension cell', async ({ page }) => {
      // Click on the dimension cell
      await dimensionTestBase.clickDimensionCell(page, testDimension.id);

      // Wait for popup to appear
      await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);
      ...
      const updateForm = dimensionTestBase.getDimensionUpdateForm(page, testDimension.id);
    ...
    test('should close popup when clicking away', async ({ page }) => {
      ...
      await dimensionTestBase.waitForDimensionPopupHidden(page, testDimension.id);
  ...
  test.describe('Text, Number, and Bool Dimension Types', () => {
    ...
    for (const dimensionTypeConfig of dimensionTypes) {
      test.describe(`${dimensionTypeConfig.name}`, () => {
        ...
        test('should contain a name text field to rename the dimension', async ({ page }) => {
          // Open the popup
          await dimensionTestBase.clickDimensionCell(page, testDimension.id);
          await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);

          // Verify name text field exists and is visible
          const nameField = dimensionTestBase.getDimensionNameField(page, testDimension.id);
          ...
          console.log(`✅ Name text field is present for ${dimensionTypeConfig.name}`);
        });
        ...
        test('should update dimension name when changing name and clicking save', async ({
          ...
          // Fill the new name
          await dimensionTestBase.fillDimensionNameField(page, testDimension.id, newName);

          // Click save
          await dimensionTestBase.clickDimensionSaveButton(page, testDimension.id);

          // Wait for the update to complete - check if the name in the cell changes
          const nameElement = page.locator(`[data-testid="dimension-name-${testDimension.id}"]`);
          ...
          console.log(`✅ Dimension name updated successfully for ${dimensionTypeConfig.name}`);
        });

        test('should show error when entering empty string and clicking save', async ({ page }) => {
          // Open the popup
          await dimensionTestBase.clickDimensionCell(page, testDimension.id);
          await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);

          // Clear the name field
          await dimensionTestBase.fillDimensionNameField(page, testDimension.id, '');
        ...
        test('should not update dimension when clicking away without saving', async ({ page }) => {
        ...
        test('should delete dimension when clicking delete button', async ({ page }) => {
          // Open the popup
          await dimensionTestBase.clickDimensionCell(page, testDimension.id);
          await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);

          // Click delete button
          await dimensionTestBase.clickDimensionDeleteButton(page, testDimension.id);
          ...
          const columnExists = await dimensionTestBase.columnExists(page, testDimension.name);
          expect(columnExists).toBe(false);

          // Mark dimension as deleted to avoid cleanup issues
          testDimension.id = '';

          console.log(`✅ Dimension deleted successfully for ${dimensionTypeConfig.name}`);
        });
      });
    }
  });
  ...
  test.describe('Dim Entries Type Dimensions', () => {
    ...
    test.beforeEach(async ({ page }) => {
      ...
      const dimensionName = `Tags Dimension ${test.info().workerIndex}-${Date.now()}`;
      testDimension = await dimensionTestBase.createTestDimension({
        name: dimensionName,
    ...
    test('should contain name text field and list of dim entry options', async ({ page }) => {
      // Open the popup
      await dimensionTestBase.clickDimensionCell(page, testDimension.id);
      await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);
      ...
      const entriesSection = dimensionTestBase.getDimensionEntriesSection(page, testDimension.id);
      await expect(entriesSection).toBeVisible();

      // Verify dimension entry items are present
      const entryItems = dimensionTestBase.getDimensionEntryItems(page, testDimension.id);
      await expect(entryItems).toHaveCount(3); // We created 3 entries

      console.log('✅ Name field and dim entries list are present for DIM_ENTRIES type');
    });

    test('should allow editing dim entry when clicking edit button', async ({ page }) => {
      // Open the popup
      await dimensionTestBase.clickDimensionCell(page, testDimension.id);
      await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);
      ...
      const editButton = dimensionTestBase.getDimensionEntryEditButton(page, extractedId);
      await editButton.click();

      // Verify edit field appears
      const editField = dimensionTestBase.getDimensionEntryEditField(page, extractedId);
      await expect(editField).toBeVisible();

      // Verify confirm and cancel buttons appear
      const confirmButton = dimensionTestBase.getDimensionEntryConfirmEditButton(page, extractedId);
      const cancelButton = dimensionTestBase.getDimensionEntryCancelEditButton(page, extractedId);
    ...
    test('should update dim entry when editing and clicking confirm', async ({ page }) => {
      // Open the popup
      await dimensionTestBase.clickDimensionCell(page, testDimension.id);
      await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);
      ...
      const entryNameElement = page.locator(`[data-testid="dim-entry-name-${extractedId}"]`);
    ...
    test('should cancel edit when clicking cancel button', async ({ page }) => {
      ...
      // Get original name
      const originalNameElement = page.locator(`[data-testid="dim-entry-name-${extractedId}"]`);
    ...
    test('should delete dim entry when clicking delete button', async ({ page }) => {
      // Open the popup
      await dimensionTestBase.clickDimensionCell(page, testDimension.id);
      await dimensionTestBase.waitForDimensionPopupVisible(page, testDimension.id);
      ...
      // Click delete button
      const deleteButton = dimensionTestBase.getDimensionEntryDeleteButton(page, extractedId);
      await deleteButton.click();

      // Verify the entry is removed from the list
      const updatedEntryItems = dimensionTestBase.getDimensionEntryItems(page, testDimension.id);
      await expect(updatedEntryItems).toHaveCount(initialCount - 1);

      // Verify the specific entry is no longer present
      const deletedEntry = dimensionTestBase.getDimensionEntryItem(page, extractedId);
