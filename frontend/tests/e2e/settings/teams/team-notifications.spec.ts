...
test.describe('Team notifications', () => {
  ...
  test('invited user receives notification when invited to a team', async ({}, testInfo) => {
    ...
    const notification = notifications.find((n) => n.type === 'user_received_team_invite');

    expect(notification).toBeDefined();

    expect(notification!.userId).toBe(user2.user_id);
    expect(notification!.teamId).toBe(team.teamId);
    expect(notification!.type).toBe('user_received_team_invite' as NotificationTypeT);
    expect(notification!.eventData.teamName).toBe(team.name);
    expect(notification!.eventData.senderName).toBe(`${user1.first_name} ${user1.last_name}`);
  ...
  test('inviter is notified when invited user accepts the invitation', async ({}, testInfo) => {
    ...
    const notification = notifications.find((n) => n.type === 'user_accepted_team_invite');

    expect(notification).toBeDefined();

    expect(notification!.userId).toBe(user1.user_id);
    expect(notification!.teamId).toBe(team.teamId);
    expect(notification!.type).toBe('user_accepted_team_invite' as NotificationTypeT);
    expect(notification!.eventData.acceptedUserName).toBe(`${user2.first_name} ${user2.last_name}`);
    expect(notification!.eventData.teamName).toBe(team.name);
  ...
  test('team owner is notified when a member leaves the team', async ({}, testInfo) => {
    ...
    expect(notification!.eventData.userName).toBe(`${user2.first_name} ${user2.last_name}`);
    expect(notification!.eventData.teamName).toBe(team.name);
  });
  ...
  test('removed member is notified when kicked from the team', async ({}, testInfo) => {
    ...
    const notifications = await dbUtils.getNotificationsAs(user2.user_id);
    const notification = notifications.find((n) => n.type === 'user_removed_from_team');

    expect(notification).toBeDefined();

    expect(notification!.userId).toBe(user2.user_id);
    expect(notification!.teamId).toBe(team.teamId);
    expect(notification!.type).toBe('user_removed_from_team' as NotificationTypeT);
    expect(notification!.eventData.teamName).toBe(team.name);
  });
});
