...
import type { NotificationKey, NotificationCategory } from '@/types/notification';
...
function keysByCategory(): Partial<Record<NotificationCategory, NotificationKey[]>> {
  return NOTIFICATION_KEYS.reduce<Partial<Record<NotificationCategory, NotificationKey[]>>>(
    (acc, key) => {
...
test.describe('Notification Settings', () => {
  ...
  test('all notification categories are visible', async ({ page }) => {
    ...
    for (const cat of NOTIFICATION_CATEGORY_ORDER) {
      await expect(page.locator(`[data-testid="notification-category-${cat}"]`)).toBeVisible();
    }
  });

  // 2 — All keys in correct categories ----------------------------------------

  test('all notification keys are visible in their correct categories', async ({ page }) => {
    await page.goto(SETTINGS_URL);
    await expect(page.locator('role=progressbar')).not.toBeVisible({
      timeout: 10_000,
    });
    ...
    for (const cat of NOTIFICATION_CATEGORY_ORDER) {
      ...
      const categoryBox = page.locator(`[data-testid="notification-category-${cat}"]`);
  ...
  test('accordion for each visible key opens and shows in-app and email switches defaulting to on', async ({
    ...
    for (const key of allKeys) {
      ...
      const { category } = NOTIFICATION_REGISTRY[key];
      const categoryBox = page.locator(`[data-testid="notification-category-${category}"]`);
      await expect(
        categoryBox.locator(`[data-testid="notification-accordion-${key}"]`),
      ).toBeVisible();

      // Click summary to expand
      await page.locator(`[data-testid="notification-accordion-summary-${key}"]`).click();

      // Both rows must appear
      await expect(page.locator(`[data-testid="notification-inapp-row-${key}"]`)).toBeVisible();
      await expect(page.locator(`[data-testid="notification-email-row-${key}"]`)).toBeVisible();
  ...
  test('accordion subtitle reflects all four channel preference combinations', async ({
    ...
    const statusLocator = page.locator(`[data-testid="notification-status-${key}"]`);
    ...
    await expect(statusLocator).toHaveAttribute('data-status', 'status_in_app_and_email');
    ...
    await expect(statusLocator).toHaveAttribute('data-status', 'status_email_only');
    ...
    await expect(statusLocator).toHaveAttribute('data-status', 'status_in_app_only');
  });
});
