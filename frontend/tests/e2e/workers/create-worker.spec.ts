...
test.describe('Worker Creation', () => {
  ...
  test('should start with an empty worker table', async ({ page }, testInfo) => {
  ...
  test('should create a new worker when "+Worker" button is pressed', async ({
    ...
    await getTestBase(testInfo).createWorkerViaUI(page);

    // Verify default worker properties based on the specifications:

    // 1. Name display should show the default placeholder
    const nameDisplay = getTestBase(testInfo).getWorkerNameDisplay(page);
    ...
    const startDisplay = getTestBase(testInfo).getWorkerEmploymentStartDisplay(page);
    ...
    const dutiesDisplay = getTestBase(testInfo).getWorkerDutiesPerMonthDisplay(page);
    await expect(dutiesDisplay).toContainText('4');

    // 7. Annual leave should default to 25
    const annualLeaveDisplay = getTestBase(testInfo).getWorkerAnnualLeaveDisplay(page);
    await expect(annualLeaveDisplay).toContainText('25');

    console.log('✅ New worker created with correct default values');
  });

  test('should allow editing the newly created worker', async ({ page }, testInfo) => {
    // First create a worker using the shared utility
    await getTestBase(testInfo).createWorkerViaUI(page);

    // Click the name cell to enter edit mode
    await getTestBase(testInfo).getWorkerNameCell(page).click();

    // Get the name input that appears in edit mode
    const nameInput = getTestBase(testInfo).getWorkerNameInput(page);
  ...
  test('should display the correct table headers', async ({ page }, testInfo) => {
