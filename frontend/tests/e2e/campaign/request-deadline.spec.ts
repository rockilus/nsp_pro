...
test("1 - owner sets deadline via UI; panel shows deadline and action buttons", async ({
  ...
  await expect(
    page.locator('[data-testid="request-deadline-panel"]'),
  ).toBeVisible({ timeout: 10_000 });
  await expect(
    page.locator('[data-testid="set-deadline-button"]'),
  ).toBeVisible();
  ...
  await page.fill(
    '[data-testid="deadline-dialog"] input[type="date"]',
    futureDate,
  );
  ...
  await expect(
    page.locator('[data-testid="send-reminder-button"]'),
  ).toBeVisible({ timeout: 10_000 });
  await expect(
    page.locator('[data-testid="extend-deadline-button"]'),
  ).toBeVisible();
});
...
test("2 - deadline seeded via API; member sees banner on request tab", async ({
  ...
  const { dbUtils, team, owner, member, scheduleId } = ctxMap.get(
    (testInfo as any).__runId,
  )!;
  ...
  await expect(
    page.locator('[data-testid="request-deadline-banner"]'),
  ).toBeVisible({ timeout: 10_000 });
});

test("3 - no deadline set; banner is absent for member", async ({
  page,
}, testInfo) => {
...
test("4 - owner sends reminder via UI; snackbar appears", async ({
  page,
}, testInfo) => {
  const { dbUtils, team, owner, scheduleId } = ctxMap.get(
    (testInfo as any).__runId,
  )!;
  ...
  await expect(
    page.locator('[data-testid="reminder-sent-snackbar"]'),
  ).toBeVisible({ timeout: 5_000 });
});

test("5 - owner extends deadline via UI; panel shows updated date", async ({
  page,
}, testInfo) => {
  const { dbUtils, team, owner, scheduleId } = ctxMap.get(
    (testInfo as any).__runId,
  ...
  await expect(
    page.locator('[data-testid="extend-deadline-dialog"]'),
  ).toBeVisible({ timeout: 5_000 });

  const newDate = dayjs.utc().add(14, "day").format("YYYY-MM-DD");
  await page.fill(
    '[data-testid="extend-deadline-dialog"] input[type="date"]',
    newDate,
  );
  await page
