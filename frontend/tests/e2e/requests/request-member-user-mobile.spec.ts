...
import { ShiftType, ShiftT } from '../../../src/types/shift';
...
test.describe('Mobile Request Page - Member User', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await roleTestBase.createWorkerForMember(`Mobile Member Worker ${testRunId}`);
    ...
    await expect(page.locator('[data-testid="mobile-request-tab"]')).toBeVisible();
  });
  ...
  test('member can create a request for their worker and it is displayed in the mobile list', async ({
    ...
    await expect(page.locator('[data-testid="request-panel-dialog"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="request-panel"]')).not.toBeVisible();
    ...
    console.log('✅ Member successfully created request for their worker (mobile)');
  });

  test('member can see their own requests in the mobile list', async ({ page }) => {
    const memberWorker = roleTestBase.getMemberWorker();
    if (!memberWorker) {
      throw new Error('Member worker not created');
    }
  ...
  test("member cannot see other workers' requests in the mobile list", async ({ page }) => {
    ...
    console.log("✅ Member cannot see other workers' requests (verified absence in mobile list)");
  });
  ...
  test('member can edit their own requests', async ({ page }) => {
    ...
    const firstRequest = page.locator('[data-testid^="mobile-request-item-"]').first();
