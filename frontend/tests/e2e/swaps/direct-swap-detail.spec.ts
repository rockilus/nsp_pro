...
test.describe("Direct Swap Detail - Team Leader Approval Tests", () => {
  ...
  test("should swap assignments correctly between workers", async ({
    ...
    const allAssignments = await swapTestBase.getAssignmentsAndRecurrences(
      includeCampaign: false,
      startDate: dayjs.utc().subtract(2, "month"),
      endDate: dayjs.utc().add(2, "month"),
    );
  ...
  test("should verify assignments maintain their shift and date after swap", async ({
    ...
    const assignmentsBefore = await swapTestBase.getAssignmentsAndRecurrences();
    ...
    const assignmentsAfter = await swapTestBase.getAssignmentsAndRecurrences();
