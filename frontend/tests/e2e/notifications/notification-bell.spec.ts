...
test.describe('NotificationBell', () => {
  ...
  test('badge shows the number of unseen notifications', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
  ...
  test('badge is hidden when there are no unseen notifications', async ({ page }, testInfo) => {
    const { dbUtils, user1 } = testContextMap.get(testContextMap.getRunId(testInfo));
  ...
  test('shows notifications in newest-first order', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
    ...
    await expect(items.nth(0)).toHaveAttribute('data-notification-type', 'user_left_team');
  ...
  test('shows notifications from all teams the user belongs to', async ({ page }, testInfo) => {
    const { dbUtils, user1 } = testContextMap.get(testContextMap.getRunId(testInfo));
  ...
  test('each notification shows message text and timestamp', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
    ...
    const timestamp = firstItem.locator('[data-testid="notification-time-since"]');
    await expect(timestamp).toBeVisible();
    await expect(timestamp).not.toBeEmpty();
  });

  test("clicking 'see all' navigates to the notifications page", async ({ page }, testInfo) => {
    const { dbUtils, user1 } = testContextMap.get(testContextMap.getRunId(testInfo));
  ...
  test('three-dots menu button is visible in popover header', async ({ page }, testInfo) => {
    const { dbUtils, user1 } = testContextMap.get(testContextMap.getRunId(testInfo));

    await navigateToPlanAndOpenBellAsUser(page, dbUtils, user1!.user_id);

    await expect(page.locator('[data-testid="notification-bell-menu-button"]')).toBeVisible();
  });

  test('badge clears after opening the bell popover', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
