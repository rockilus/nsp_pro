...
test.describe('NotificationsPage', () => {
  ...
  test('three-dots menu button is visible in page header', async ({ page }, testInfo) => {
    const { dbUtils, user1 } = testContextMap.get(testContextMap.getRunId(testInfo));

    await navigateToNotificationsAsUser(page, dbUtils, user1!.user_id);

    await expect(page.locator('[data-testid="notifications-page-menu-button"]')).toBeVisible();
  });

  test('mark all as read from menu marks all notifications as read', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
  ...
  test('clicking a notification navigates to the correct page', async ({ page }, testInfo) => {
    const { dbUtils, team, user1, user2 } = testContextMap.get(testContextMap.getRunId(testInfo));
