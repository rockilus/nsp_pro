...
test.describe('ScheduleNavBar - Owner Tests', () => {
  ...
  test.describe('Time Frame Selection', () => {
    test('should display monthly view when month is selected', async ({ page }) => {
      ...
      const displayedMonthStart = dayjs.utc(scheduleViewSettings.periodStartDate);
    ...
    test('should display weekly view when week is selected', async ({ page }) => {
      ...
      const displayedWeekStart = dayjs.utc(scheduleViewSettings.periodStartDate);
      ...
      const expectedLabel = formatPeriodLabel(displayedWeekStart, displayedWeekEnd, 'week', 'en');
  ...
  test.describe('Time Navigation - Weekly View', () => {
    ...
    test('should navigate to next week when next button is pressed', async ({ page }) => {
      // Get initial period
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      ...
      expect(newWeekStart.format('YYYY-MM-DD')).toBe(expectedWeekStart.format('YYYY-MM-DD'));
    ...
    test('should navigate to previous week when previous button is pressed', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      expect(afterWeekStart.format('YYYY-MM-DD')).toBe(expectedWeekStart.format('YYYY-MM-DD'));
    ...
    test('should navigate back to current week when today button is pressed', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
  ...
  test.describe('Time Navigation - Monthly View', () => {
    ...
    test('should navigate to next month when next button is pressed', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      const expectedMonthStart = initialMonthStart.add(1, 'month').startOf('month');

      // Verify the new month is the next month
      expect(newMonthStart.format('YYYY-MM')).toBe(expectedMonthStart.format('YYYY-MM'));
    ...
    test('should navigate to previous month when previous button is pressed', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      const expectedMonthStart = beforeMonthStart.subtract(1, 'month').startOf('month');

      // Verify the new month is the previous month
      expect(afterMonthStart.format('YYYY-MM')).toBe(expectedMonthStart.format('YYYY-MM'));
    ...
    test('should navigate back to current month when today button is pressed', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      expect(currentMonthStart.format('YYYY-MM')).toBe(expectedMonthStart.format('YYYY-MM'));
  ...
  test.describe('Period Label Display', () => {
    test('should display correct month name and year in monthly view', async ({ page }) => {
      // Ensure monthly view
      const timeFrameSelect = page.locator('[data-testid="time-nav-select"]');
      await timeFrameSelect.selectOption('month');
      await page.waitForTimeout(500);
    ...
    test('should display correct period in weekly view', async ({ page }) => {
      ...
      console.log(`✅ Week within same month displays correctly: "${labelText}"`);
      ...
      console.log(`✅ Week spanning months (same year) displays correctly: "${labelText}"`);
    });
  });

  test.describe('Data View Selection', () => {
    test('should display shift table when shift view is selected', async ({ page }) => {
    ...
    test('should display worker table when worker view is selected', async ({ page }) => {
  ...
  test.describe('Settings Popover', () => {
    test('should open popover when settings button is clicked', async ({ page }) => {
    ...
    test('should display shift view when shift is selected in settings', async ({ page }) => {
      // Open settings
      const settingsButton = page.locator('[data-testid="schedule-settings-button"]');
      await settingsButton.click();
      ...
      const shiftButton = popover.locator('[data-testid="settings-groupby-shift"]');
      await shiftButton.click();
      await page.waitForTimeout(500);
    ...
    test('should display worker view when worker is selected in settings', async ({ page }) => {
      // Open settings
      const settingsButton = page.locator('[data-testid="schedule-settings-button"]');
      await settingsButton.click();
      ...
      const workerButton = popover.locator('[data-testid="settings-groupby-worker"]');
      await workerButton.click();
      await page.waitForTimeout(500);
    ...
    test('should show/hide assignments when checkbox is toggled', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      const assignmentsCheckbox = popover.locator('[data-testid="settings-checkbox-assignments"]');
    ...
    test('should show/hide demands when checkbox is toggled in shift view', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      const demandsCheckbox = popover.locator('[data-testid="settings-checkbox-demands"]');
    ...
    test('should show/hide requests when checkbox is toggled in worker view', async ({ page }) => {
      const testTeam = scheduleTestBase.getTestTeam();
      if (!testTeam) {
        throw new Error('Test team not found');
      }
      ...
      const requestsCheckbox = popover.locator('[data-testid="settings-checkbox-requests"]');
    ...
    test('should enable duplicate week only in weekly view', async ({ page }) => {
      ...
      // Open settings
      let settingsButton = page.locator('[data-testid="schedule-settings-button"]');
      await settingsButton.click();
      ...
      const duplicateWeekButton = popover.locator('[data-testid="settings-duplicate-week-button"]');
      ...
      console.log(
        `✅ Duplicate week button is ${isDisabledInWeek ? 'disabled' : 'enabled'} in weekly view`,
      );
    });

    test('should open duplicate dialog when duplicate week is clicked', async ({ page }) => {
      // Switch to weekly view
      const timeFrameSelect = page.locator('[data-testid="time-nav-select"]');
      await timeFrameSelect.selectOption('week');
      await page.waitForTimeout(500);
  ...
  test.describe('Campaign Information', () => {
    test('should display campaign start and end dates', async ({ page }) => {
      ...
      const campaignDates = page.locator('[data-testid="campaign-period-dates"]');
    ...
    test('should open breaches in dialog when campaign status is clicked', async ({ page }) => {
      ...
      if (chipCount > 0) {
        ...
        console.log('⏭️  No campaign status chip found (team may not use solver)');
      }
    });
  ...
  test.describe('No Campaign Scenario', () => {
    ...
    test('should show create campaign button when no campaign exists', async ({ page }) => {
      ...
      const createCampaignButton = page.locator('[data-testid="nav-bar-create-campaign-button"]');
      await expect(createCampaignButton).toBeVisible();

      console.log('✅ Create campaign button displayed when no campaign exists');
    });
