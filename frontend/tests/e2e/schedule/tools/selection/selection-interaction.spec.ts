...
async function getSelectionCountsText(page: import('@playwright/test').Page): Promise<string> {
  return (await page.locator('[data-testid="schedule-selection-counts"]').textContent()) ?? '';
}
...
for (const groupBy of ['shift', 'worker'] as const) {
  test.describe(`Schedule Selection Interaction (groupBy=${groupBy})`, () => {
    ...
    test('select/unselect individual assignment', async ({ page }, testInfo) => {
      ...
      const result = await scheduleTestBase.getAssignmentsAndRecurrences(false, startDate, endDate);
      ...
      const assignmentCell = page.locator(`[data-testid="assignment-cell-${assignmentId}"]`);
    ...
    test('column checkbox selects/deselects all items in that column', async ({
      ...
      expect(countsAfterSelect.includes('cell') || countsAfterSelect.includes('assignment')).toBe(
        true,
      );
    ...
    test('row checkbox (view scope) selects/deselects all in row', async ({ page }, testInfo) => {
      const testRunId = (testInfo as any).testRunId as string;
      const scheduleTestBase = testBasesMap.get(testRunId)!;
    ...
    test.describe(`with campaign (groupBy=${groupBy})`, () => {
      test('row checkbox (campaign scope) selects across campaign', async ({ page }, testInfo) => {
        const testRunId = (testInfo as any).testRunId as string;
        const scheduleTestBase = testBasesMap.get(testRunId)!;
      ...
      test('switching scope deselects out-of-range cells', async ({ page }, testInfo) => {
        const testRunId = (testInfo as any).testRunId as string;
        const scheduleTestBase = testBasesMap.get(testRunId)!;
      ...
      test('switching scope deselects out-of-scope cells when view is outside campaign', async ({
        ...
        const campaignCountsText = await getSelectionCountsText(page);
        expect(
          campaignCountsText.includes('cell') || campaignCountsText.includes('assignment'),
        ).toBe(true);
        ...
        const viewCountsText = await getSelectionCountsText(page);
        expect(viewCountsText.includes('cell') || viewCountsText.includes('assignment')).toBe(true);
