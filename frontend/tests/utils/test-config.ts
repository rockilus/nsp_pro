...
if (!isCI) {
  ...
  if (result.error) {
    ...
    console.log('Test environment configuration loaded successfully from .env.test.local');
  }
} else {
...
export function loadTestConfig(): TestConfig {
  ...
  const config: TestConfig = {
    ...
    confirmationToken: process.env.TEST_CONFIRMATION_TOKEN || 'test-reset-confirm',
    dbResetTimeoutMs: parseInt(process.env.TEST_DB_RESET_TIMEOUT_MS || '30000', 10),
    apiReadyTimeoutMs: parseInt(process.env.TEST_API_READY_TIMEOUT_MS || '10000', 10),
  };
