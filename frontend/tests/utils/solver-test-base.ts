...
export class SolverTestBase {
  ...
  async loadScenario(scenarioName: string): Promise<SolverScenarioResult> {
    ...
    const scenario = await this.dbUtils.loadSolverScenario(scenarioName, this.testTeam.teamId);
   ...
   */
  async createSchedule(startDate: string, endDate: string): Promise<{ scheduleId: string }> {
    if (!this.testTeam) {
   ...
   */
  async triggerSolveAndWait(page: Page, timeoutMs: number = 60000): Promise<void> {
    ...
    const errorSnackbar = await page.locator('[data-testid="solve-error-snackbar"]');
    const successSnackbar = await page.locator('[data-testid="solve-success-snackbar"]');

    const isErrorVisible = await errorSnackbar.isVisible().catch(() => false);
    const isSuccessVisible = await successSnackbar.isVisible().catch(() => false);

    if (isErrorVisible) {
      const errorAlert = await page.locator('[data-testid="solve-error-alert"]');
    ...
    if (isSuccessVisible) {
      console.log('ℹ️  Success notification visible, waiting for it to close...');
    ...
    for (const status of validStatuses) {
      const statusChip = await page.locator(`[data-testid="solve-status-chip-${status}"]`);
    ...
    if (!statusFound) {
      throw new Error(`Expected solve status chip to show one of: ${validStatuses.join(', ')}`);
    }
   ...
   */
  async verifySolveResults(page: Page, expectedCriteria: VerifyCriteria): Promise<void> {
    ...
    if (expectedCriteria.minAssignments !== undefined) {
      expect(assignmentCount).toBeGreaterThanOrEqual(expectedCriteria.minAssignments);
    ...
    if (expectedCriteria.coveragePercentage !== undefined) {
      ...
      if (coverageText) {
        ...
        expect(coverage).toBeGreaterThanOrEqual(expectedCriteria.coveragePercentage);
   ...
   */
  async takeScreenshot(page: Page, scenarioName: string, description: string = ''): Promise<void> {
   ...
   */
  async setupScopedSolveScenario(teamId: string): Promise<ScopedSolveFixtureResult> {
  ...
  getCurrentFixture(): ScopedSolveFixtureResult {
    if (!this.currentFixture) {
      throw new Error('No scoped solve fixture loaded. Call setupScopedSolveScenario first.');
    }
  ...
  async selectSolveScope(
    ...
    const menuItem = page.locator(`[data-testid="solve-scope-menu-item-${scope}"]`);
  ...
  async triggerCustomSolveInShiftView(
    ...
    const normDate = (d: any) => {
      if (typeof d === 'number') return dayjs.unix(d).utc().format('YYYY-MM-DD');
    ...
    for (const shiftId of shiftIds) {
      const rowBtn = page.locator(`[data-testid="shift-row-custom-select-${shiftId}"]`);
    ...
    for (const d of dates) {
      ...
      const dateBtn = page.locator(`[data-testid="date-column-sparkle-${dateStr}"]`);
    ...
    if (shiftCells.length > 0) {
      for (const c of shiftCells) {
        ...
        if (match && (match as any).id) {
          const dsdBtn = page.locator(`[data-testid="dsd-custom-select-${(match as any).id}"]`);
    ...
    const confirmBtn = page.locator('[data-testid="custom-solve-confirm-button"]');
  ...
  async triggerCustomSolveInWorkerView(
    ...
    for (const workerId of workerIds) {
      const rowBtn = page.locator(`[data-testid="worker-row-custom-select-${workerId}"]`);
      await rowBtn.waitFor({ state: 'visible', timeout: 5000 });
      await rowBtn.click();
    }
  ...
  areAssignmentsFulfillingScope(
    ...
    const shiftDemandsInCampaign = shiftDemands.filter((d) => {
      const dd = typeof d.date === 'number' ? dayjs.unix(d.date).utc() : dayjs.utc(d.date);
    ...
    if (scopeType === 'FULL') {
      ...
      for (const s of svals) {
        if (s.shiftType === ShiftType.DUTY || s.restType === ShiftRestType.RECUPERATION) {
          dutyIds.add(s.id);
        }
      }
      ...
      if (solveView === 'shift') {
        ...
        if (dates.length > 0) includeIf((d) => dates.includes(normDate(d.date)));
        if (shiftCells.length > 0) {
          for (const c of shiftCells) {
        ...
        const workerCellSet = new Set<string>(
          (workerCells || []).map((c) => `${c.worker_id || c.workerId}|${normDate(c.date)}`),
        );
        ...
        if (workerCellSet.size > 0) {
          includeIf((d) => workerCells.some((c) => normDate(c.date) === normDate(d.date)));
        }
    ...
    const isCustomWorkerView =
      scopeType === 'CUSTOM' && (solveScope?.solve_view || 'shift') === 'worker';
    ...
    for (const a of assignmentsInCampaign) {
      ...
      if (isCustomWorkerView) {
        ...
        const allowedByWorker = workerIdSet.size > 0 && workerIdSet.has(a.workerId);
        const allowedByDate = dateSet.size > 0 && dateSet.has(dateStr);
        const allowedByCell =
          workerCellSet.size > 0 && workerCellSet.has(`${a.workerId}|${dateStr}`);
        if (!allowedByWorker && !allowedByDate && !allowedByCell) {
          throw new Error(`Assignment (${a.workerId}, ${dateStr}, ${a.shiftId}) is not in scope`);
        }
      } else {
