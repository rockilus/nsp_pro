...
export class SwapTestBase {
  ...
  private async createTestAssignments(referenceDate: dayjs.Dayjs): Promise<void> {
    ...
    if (campaignSchedule) {
      ...
      for (const worker of this.testWorkers) {
        const ARResult1 = await this.dbUtils.createAssignmentAndRecurrence({
          teamId: this.testTeam.teamId,
          workerId: worker.id,
          shiftId: this.testShifts[0].id,
          date: campaignDate1,
        ...
        const ARResult2 = await this.dbUtils.createAssignmentAndRecurrence({
          teamId: this.testTeam.teamId,
          workerId: worker.id,
  ...
  /**
   * Select the "All Swaps" tab in the Swap UI (leader only)
   */
  async selectAllSwapsTab(page: Page): Promise<void> {
    await page.waitForSelector('[data-testid="filter-all-swaps"]', {
      timeout: 5000,
    });
    await page.click('[data-testid="filter-all-swaps"]');
    // Give the UI a short moment to update
    await page.waitForTimeout(500);
  }
