...
export default function NotificationSettingsTab({ lng }: { lng: string }) {
  ...
  const handleChange = (key: NotificationKey, channel: 'inApp' | 'email', value: boolean) => {
    const updated = {
      ...prefs,
      preferences: {
        ...prefs.preferences,
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mt: 3 }}>
        {NOTIFICATION_CATEGORY_ORDER.filter((cat) => (keysByCategory[cat]?.length ?? 0) > 0).map(
          (cat) => (
            <Box key={cat} data-testid={`notification-category-${cat}`} sx={{ mb: 3 }}>
                      ...
                      <AccordionDetails>
                        <FormControlLabel
                          ...
                          control={
                            <Switch
                              checked={ch.inApp}
                              onChange={(e) => handleChange(key, 'inApp', e.target.checked)}
                            />
                          }
