...
import { Select, MenuItem, FormControl, InputLabel, IconButton, Button, TextField, SelectChangeEvent } from '@mui/material';
...
const Filter: React.FC<FilterProps> =  ({ filter, onRemoveFilter }) => {
  return (
    <div>
...
const NewFilter: React.FC<NewFilterProps> =  ({ data, onAddFilter }) => {
  ...
  const handleValueChange = (event: SelectChangeEvent<HTMLInputElement | HTMLTextAreaElement> | ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | Event & { target: { value: string; name: string; }; }) => {
...
type QueryFilterProps = {
  data: Config;
  onFiltersReady: (filters: any[]) => void;
};

const QueryFilter: React.FC<QueryFilterProps> = ({ data, onFiltersReady }) => {
  const [filters, setFilters] = useState<QueryFilter[]>([]);

  const handleAddFilter = (newFilter: QueryFilter) => {
