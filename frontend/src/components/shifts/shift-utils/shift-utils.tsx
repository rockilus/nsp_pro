...
// Types
import { ShiftLeaveType, ShiftRestType, ShiftT, ShiftType } from '../../../types/shift';
...
export const useRestNameDisplayed = ({ lng }: { lng: string }) => {
  ...
  const restShiftDisplayNames: {
    [key in Exclude<ShiftRestType, ShiftRestType.NONE | ShiftRestType.RECUPERATION>]: string;
  } = {
    [ShiftRestType.OFF]: t('off'),
  };

  const getRestNameDisplayed = (
    restType: Exclude<ShiftRestType, ShiftRestType.NONE | ShiftRestType.RECUPERATION>,
  ): string => {
