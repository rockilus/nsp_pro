...
import { SolveScope, WorkerDateCell, ShiftDateCell } from '../../../types/solveTaskStatus';
...
export default function CustomSolveDialog({
  ...
  const selectedEntities = useMemo(() => {
    const activeCells = groupBy === 'worker' ? workerSolveCells : shiftSolveCells;
    ...
    const entities: (WorkerT | ShiftT)[] = groupBy === 'worker' ? workers : shifts;
    ...
    return entities
      ...
      .map((e) => {
        const selectedDates = [...(rowMap.get(e.id) ?? [])]
          ...
          .sort();
        const isFull = campaignDates.length > 0 && selectedDates.length >= campaignDates.length;
        return {
          rowId: e.id,
          name: e.name,
          isFull,
  ...
  }, [workerSolveCells, shiftSolveCells, scheduleCampaign, groupBy, workers, shifts]);
  ...
  return (
    <Dialog data-testid="custom-solve-dialog" open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        {isEmpty ? (
          ...
          >
            {selectedEntities.map(({ rowId, name, isFull, dates, colorKey }) => {
              const colorMapping = colorKey ? ShiftColorMappings[colorKey] : undefined;
              return (
                <Box
                  key={rowId}
                  sx={{
