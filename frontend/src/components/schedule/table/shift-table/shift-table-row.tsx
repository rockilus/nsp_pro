...
export default function ShiftTableRow({
  ...
  const campaignDates = scheduleCampaign
    ...
    : [];
  const isRowCustomSelected = isCustomSolveModeActive && campaignDates.length > 0;
  const isRowCustomIndeterminate =
    isCustomSolveModeActive &&
    campaignDates.length > 0 &&
    campaignDates.length < customSolveSelectedCells.filter((c) => c.rowId === shift.id).length;
