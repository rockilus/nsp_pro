...
import { ColumnFilter, TableSort, TableState, ColumnDefinition } from '../types/filter';
  ...
  switch (type) {
    ...
    case 'select':
      return Array.isArray(value)
        ? value.every((v) => typeof v === 'string' && v.length <= 100) && value.length <= 50
...
function isValidSort(sort: any): sort is TableSort {
  ...
  return (
    typeof columnId === 'string' && columnId.length <= 100 && ['asc', 'desc'].includes(direction)
  );
}
...
function loadTableState(storageKey: string, columns: ColumnDefinition[]): TableState {
  try {
...
export function useTableState<T>(data: T[], columns: ColumnDefinition[], storageKey?: string) {
  ...
  const filteredAndSortedData = useMemo(() => {
          ...
          case 'select':
            if (Array.isArray(filter.value)) {
              ...
              if (Array.isArray(value)) {
                ...
                return filter.value.every((filterVal) => value.includes(filterVal));
              } else {
    ...
    // Apply sorting
    if (tableState.sort) {
      const column = columns.find((col) => col.id === tableState.sort!.columnId);
      if (column) {
        result.sort((a, b) => {
