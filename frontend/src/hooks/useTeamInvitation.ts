...
// Types
import { TeamInvitationT, EnrichedTeamInvitationT } from '../types/team-invitation';
...
export function useCreateTeamInvitation() {
  ...
  const createTeamInvitation = useCallback(
    async (invitation: TeamInvitationT, teamId: string): Promise<TeamInvitationT> => {
      ...
      try {
        const result = await TeamInvitationApi.createTeamInvitation(apiClient, invitation, teamId);
...
export function useGetUserPendingInvitations() {
  ...
  const getUserPendingInvitations = useCallback(async (): Promise<EnrichedTeamInvitationT[]> => {
    if (env.isDevelopment) {
...
export function useAcceptTeamInvitation() {
  ...
  const acceptTeamInvitation = useCallback(
    async (token: string): Promise<TeamWithMembership> => {
      ...
      try {
        const result = await TeamInvitationApi.acceptTeamInvitation(apiClient, token);
...
export function useDeleteTeamInvitation() {
  ...
  const deleteTeamInvitation = useCallback(
    async (invitationId: string, teamId: string): Promise<{ message: string }> => {
      if (env.isDevelopment) {
