...
export function usePeriodState() {
  ...
  const [currentDate, setCurrentDateState] = useState<Dayjs>(() => dayjs(loadState().currentDate));
  const [periodType, setPeriodTypeState] = useState<PeriodType>(() => loadState().periodType);
