...
export class LinkShiftApi extends BaseApi {
  ...
  static async createLinkShiftLegacy(
    linkShift: LinkShiftT
  ): Promise<LinkShiftT> {
    console.warn("⚠️ Using legacy unauthenticated API call");
    return this.makeFetchRequest<LinkShiftT>(
      `/link-shifts/teams/${linkShift.teamId}`,
      {
        method: "POST",
  ...
  static async getLinkShiftsLegacy(teamId: string): Promise<LinkShiftT[]> {
    console.warn("⚠️ Using legacy unauthenticated API call");
    return this.makeFetchRequest<LinkShiftT[]>(`/link-shifts/teams/${teamId}`);
  }

  static async updateLinkShiftLegacy(
    linkShift: LinkShiftT
  ): Promise<LinkShiftT> {
    console.warn("⚠️ Using legacy unauthenticated API call");
    return this.makeFetchRequest<LinkShiftT>(
  ...
  static async deleteLinkShiftLegacy(
    linkShiftId: string,
    teamId: string
  ): Promise<boolean> {
    console.warn("⚠️ Using legacy unauthenticated API call");
    try {
      ...
      console.error("Legacy delete link shift failed:", error);
      return false;
    }
  }
}
