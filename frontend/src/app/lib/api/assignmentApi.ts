...
export class AssignmentApi extends BaseApi {
  ...
  static async addAssignmentAndRecurrenceLegacy(
    assignment: AssignmentT,
    recurrence: RecurrenceRuleT | null = null
  ): Promise<AssignmentsRecurrencesResultT> {
    console.warn("⚠️ Using legacy unauthenticated API call");
    return this.makeFetchRequest<AssignmentsRecurrencesResultT>(
      `/assignments/teams/${assignment.teamId}`,
      {
        method: "POST",
        body: JSON.stringify({
  ...
  static async getAssignmentsByDatesLegacy(
    teamId: string,
    startDate?: dayjs.Dayjs,
    endDate?: dayjs.Dayjs
  ): Promise<AssignmentsRecurrencesResultT> {
    ...
    return this.makeFetchRequest<AssignmentsRecurrencesResultT>(endpoint, {
      credentials: "include",
    });
  }

  static async getValidatedAssignmentsLegacy(
    teamId: string,
    startDate?: dayjs.Dayjs,
    endDate?: dayjs.Dayjs
  ): Promise<AssignmentT[]> {
    ...
    const data = await this.makeFetchRequest<any[]>(endpoint, {
      credentials: "include",
    });
    return data.map(toAssignmentT);
  }

  static async updateAssignmentAndRecurrenceLegacy(
    assignment: AssignmentT,
    teamId: string,
    recurrenceRule: RecurrenceRuleT | null = null,
    recurrenceUpdateScope: RecurrenceUpdateScope | null = null
  ...
  static async deleteAssignmentLegacy(
    assignmentId: string,
    teamId: string,
    recurrenceId: string | null = null,
    recurrenceUpdateScope: RecurrenceUpdateScope | null = null
