...
export class SqsSolveApi extends BaseApi {
  ...
  static async startSolve(
    ...
    if (!request || !request.schedule_id || !request.team_id) {
      throw new Error('Invalid solve request: schedule_id and team_id are required');
    }
   ...
   */
  static async cancelSolve(apiClient: AuthenticatedApiClient, solveId: string): Promise<void> {
    // Security: Input validation
    if (!solveId) {
      throw new Error('Solve ID is required');
    }

    await this.makeRequest<void>(apiClient, 'post', `/sqs-solve/${solveId}/cancel`);
  }
