...
export default function Draft() {
  ...
  const findShiftName = (s: any) => {
    ...
    const shiftPropForShiftName = s.shift_properties.find((p: any) => p.shift_param === shiftNameParamId)
    if (!shiftPropForShiftName) {
      throw Error(`Shift should have property for param id ${shiftNameParamId}`)
    }

    return shiftPropForShiftName.value
  }

  const shiftsForCoverage = shiftsContext.currentShifts ? shiftsContext.currentShifts.map((s) => {
    const shift: ShiftT = {
      id: s.shift._id,
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column" }}>
      ...
      <WorkerConfig />
      <ShiftConfig />
      {shiftsForCoverage.length > 0 && <CoveragePanel shifts={shiftsForCoverage}/>}
      <ConstraintConfig />
    </Box>
  );
}
