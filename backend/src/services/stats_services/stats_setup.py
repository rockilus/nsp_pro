...
import numpy as np
from core import Schedule, Stat, Stats, Assignment
...
def stats_setup(
    ...
    work_shift_to_i = {
        shift.id: i
        for i, shift in enumerate([s for s in shifts if not s.is_time_off])
    }
    ...
    rest_shift_to_i = {
        shift.id: i
        for i, shift in enumerate([s for s in shifts if s.is_time_off])
    }
            ...
            max_schedule_date,
            {
                ...
                for i, date in enumerate(
                    [
                        ...
                        for i in range(
                            (max_schedule_date - min_schedule_date).days + 1
                        )
                    ]
                )
            },
...
def build_stats_nb_days_worked(
    ...
    if target_column == "year":
        ...
        return np_to_core_days_worked_per_year(
            stats_array, i_to_worker, year_to_i
        )
