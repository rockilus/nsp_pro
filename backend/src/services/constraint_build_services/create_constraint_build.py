...
from core import Block, ConstraintBuild, MissingProperty, ShiftDimension
...
def build_missing_properties_list_and_active(
    ...
    active_shift = False
    for block in blocks:
        if block.name == "worker" and isinstance(block.value, list):
            if all(isinstance(b, dict) for b in block.value):
                wd_ids = [
                    b["id"]  # type: ignore
