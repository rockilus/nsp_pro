...
def cb_to_cb_augmented(cb: ConstraintBuild) -> ConstraintBuildAugmented:
    workers = worker_db.get_workers(cb.team_id)
...
def build_missing_attributes_and_active_dimension_bool(
    ...
    o_not_deleted = [o for o in owners if not o.deleted]
    a_all = [
        a
        for a in attributes
