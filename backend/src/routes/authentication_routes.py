...
from routes.api_model import Token as TokenMessage
...
async def register_user(response: Response, req: UserMessage) -> UserMessage:
    u_data = api_msg_to_user(req)
    user_exist = user_db.get_user_by_username(u_data.username)
...
@router.post("/token", response_model=TokenMessage)
...
@router.get("/users/me/", response_model=UserMessage)
