...
from routes.api_model import RestShiftMessage, ShiftPropertyMessage
...
@router.post("/rest-shifts/teams/{team_id}", status_code=201)
async def create_rest_shift(
    team_id: str,
    rest_shift: RestShiftMessage,
    ...
    try:
        ...
        s_data = msg_to_core_to_shift(rest_shift)
        ...
        response = core_to_msg_rest_shift_and_properties(shift_created, sp_bool)
...
@router.get("/rest-shifts/teams/{team_id}")
async def get_rest_shifts(
    team_id: str,
    session: SessionContainerType = Depends(authn_verify_session()),
) -> List[RestShiftMessage]:
    try:
        ...
        response = [
            core_to_msg_rest_shift_and_properties(s, sp)
...
@router.put("/rest-shifts/{shift_id}/teams/{team_id}")
async def update_shift(
    team_id: str,
    shift: RestShiftMessage,
    session: SessionContainerType = Depends(authn_verify_session()),
) -> RestShiftMessage:
    try:
        if not await authz_check(
        ...
        response = core_to_msg_rest_shift_and_properties(
...
@router.put("/rest-shifts/{shift_id}/properties/{shift_dimension_id}/teams/{team_id}")
...
@router.delete("/rest-shifts/{shift_id}/teams/{team_id}")
...
def core_to_msg_rest_shift_and_properties(
    ...
    as_dict = humps.camelize(data)
    validator = TypeAdapter(RestShiftMessage)
    try:
        rs_msg = validator.validate_python(as_dict)
    except Exception as e:
        log_info("Failed to convert Shift to RestShiftMessage")
        handle_message_errors(e)
    return rs_msg


# message to core
def msg_to_core_to_shift(msg: RestShiftMessage) -> Shift:
    data_snake = humps.decamelize(msg.model_dump())
    data_snake = {k: v for k, v in data_snake.items() if k != "shift_properties"}
    try:
        shift = Shift(**data_snake)
    except Exception as e:
        log_info("Failed to convert RestShiftMessage to Shift")
