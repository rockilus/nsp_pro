...
class WorkerDimensionDB:
    ...
    def create_worker_dimension(
        ...
        try:
            ...
            log_info("Failed to save worker dimension to database")
    ...
    def get_worker_dimension_by_id(
        ...
        try:
            ...
            log_info("Failed to get worker dimension by id from database")
            handle_get_document_error(e)
    ...
    def get_worker_dimension_by_name(
        ...
        try:
            ...
            log_info("Failed to get worker dimension by name from database")
            handle_get_document_error(e)
        return doc_to_core_worker_dimension(worker_dimension)
    ...
    def get_worker_dimensions_by_entry_type(
        ...
        try:
            ...
            log_info(
                "Failed to get worker dimensions by entry type from database"
            )
            handle_get_document_error(e)
        return [
            doc_to_core_worker_dimension(wd) for wd in list(worker_dimensions)
    ...
    def update_worker_dimension(
        ...
        try:
            # pylint: disable=no-member
            WorkerDimensionDocument.objects.get(id=wd_doc.id)  # type: ignore
        except Exception as e:
            log_info(f"Worker dimension with id {wd_doc.id} does not exist")
            handle_get_document_error(e)
        try:
            wd_saved = wd_doc.save()
        except Exception as e:
            log_info("Failed to update worker dimension to database")
            handle_save_document_error(e)
        return doc_to_core_worker_dimension(wd_saved)
    ...
    def delete_worker_dimension(self, worker_dimension_id: str) -> None:
        try:
            ...
            log_info(
                "Failed to get worker dimension by id to delete from database"
            )
            handle_get_document_error(e)
        try:
...
def core_to_doc_worker_dimension(
    ...
    try:
        ...
        log_info("Failed to get team by id to create worker dimension")
        handle_get_document_error(e)
    try:
        ...
        log_info(
            "Failed to convert WorkerDimension to WorkerDimensionDocument"
        )
...
def doc_to_core_worker_dimension(
    ...
    try:
        ...
        log_info(
            "Failed to convert WorkerDimensionDocument to WorkerDimension"
        )
