...
class CoverageSelectorDB:
    ...
    def get_coverage_selector_by_id(
        ...
        try:
            ...
            log_info(
                f"Failed to get coverage selector by id from database: {e}"
            )
            handle_get_document_error(e)
    ...
    def get_coverage_selector_by_dates(
        ...
        try:
            ...
            log_info(
                f"Failed to get coverage selectors by dates from database: {e}"
            )
            handle_get_document_error(e)
        return [
            doc_to_core_coverage_selector(cs)
...
def core_to_doc_coverage_selector(
    ...
    try:
        ...
        log_info(
            f"Failed to convert CoverageSelector to CoverageSelectorDocument: {e}"
        )
...
def doc_to_core_coverage_selector(
    ...
    try:
        ...
        log_info(
            f"Failed to convert CoverageSelectorDocument to CoverageSelector: {e}"
        )
