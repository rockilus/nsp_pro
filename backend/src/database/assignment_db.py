...
class AssignmentDB:
    ...
    def create_assignment(self, assignment: Assignment) -> Assignment:
        ...
        try:
            ...
            log_error(f"Failed to save assignment document to database: {e}")
    ...
    def get_assignments(self, schedules: List[Schedule]) -> List[Assignment]:
        ...
        try:
            ...
            log_error(f"Failed to get assignments from database: {e}")
    ...
    def get_assignment_by_id(self, assignment_id: str) -> Assignment:
        try:
            ...
            log_error(f"Failed to get assignment by id from database: {e}")
            handle_get_document_error(e)
    ...
    def get_assignments_by_dates(
        ...
        try:
            ...
            log_error(f"Failed to get assignments by dates from database: {e}")
            handle_get_document_error(e)
        return [_from_mongo_assignment(a) for a in list(assignments)]
    ...
    def get_assignments_by_schedule_id(self, schedule_id: str) -> List[Assignment]:
        try:
            ...
            log_error(f"Failed to get assignments by schedule id from database: {e}")
            handle_get_document_error(e)
        return [_from_mongo_assignment(a) for a in list(assignments)]
    ...
    def get_assignments_by_status(
        ...
        try:
            ...
            log_error(f"Failed to get assignments by status from database: {e}")
            handle_get_document_error(e)
        return [_from_mongo_assignment(a) for a in list(assignments)]
    ...
    def update_assignment(self, assignment: Assignment) -> Assignment:
        ...
        try:
            ...
            log_error(f"Failed to update assignment: {e}")
            handle_save_document_error(e)
        return _from_mongo_assignment(a_saved)
    ...
    def delete_assignment(self, assignment_id: str) -> None:
        try:
            ...
            log_error(f"Failed to get assignment by id to delete: {e}")
            handle_get_document_error(e)
        try:
            assignment.delete()
        except Exception as e:
            log_error(f"Failed to delete assignment: {e}")
    ...
    def delete_assignments_by_schedule_id(self, schedule_id: str) -> None:
        try:
            ...
            log_error(f"Failed to get assignments by schedule id to delete: {e}")
            handle_get_document_error(e)
        try:
            for a in assigments:
                a.delete()
        except Exception as e:
            log_error(f"Failed to delete assignments: {e}")
            handle_delete_document_error(e)
    ...
    def delete_assignments_by_worker_id(self, worker_id: str) -> None:
        try:
            ...
            log_error(f"Failed to get assignments by worker id to delete: {e}")
            handle_get_document_error(e)
        try:
...
def to_mongo_assignment(dataclass_obj: Assignment) -> AssignmentDocument:
    try:
        ...
        log_error(f"Failed to get worker by id: {e}")
        handle_get_document_error(e)
    try:
        # pylint: disable=no-member
        shift = ShiftDocument.objects.get(id=dataclass_obj.shift_id)  # type: ignore
    except Exception as e:
        log_error(f"Failed to get shift by id: {e}")
        handle_get_document_error(e)
    try:
        # pylint: disable=no-member
        ...
        log_error(f"Failed to get schedule by id: {e}")
        handle_get_document_error(e)
    try:
    ...
    # pylint: disable=broad-except
        ...
        log_error(f"Failed to convert Assignment to AssignmentDocument: {e}")
...
def _from_mongo_assignment(doc_obj: AssignmentDocument) -> Assignment:
    try:
        ...
        log_error(f"Failed to convert AssignmentDocument to Assignment: {e}")
