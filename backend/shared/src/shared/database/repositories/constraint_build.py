...
class ConstraintBuildRepository(BaseRepository[ConstraintBuildSchema]):
    ...
    def __init__(self, database_interface: DatabaseInterface):
        super().__init__(
            database_interface, "constraint_builds", ConstraintBuildSchema
        )
    ...
    def create_constraint_build(
        ...
        constraint_build_schema = ConstraintBuildSchema.from_core(
            constraint_build
        )
    ...
    def get_constraint_build_by_id(
        ...
        if not constraint_build:
            raise Exception(
                f"Constraint build with id {constraint_build_id} not found"
            )
    ...
    def get_constraint_builds_by_ids(
        ...
        constraint_builds = self.find_all(
            {"_id": {"$in": constraint_build_ids}}
        )
        return [cb.to_core() for cb in constraint_builds]
