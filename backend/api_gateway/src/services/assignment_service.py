...
class AssignmentService(BaseService):
    ...
    def _handle_recurrence_and_assignments_creation(
        ...
        if recurrence.recurrence_end_type == RecurrenceEndType.NEVER:
            ...
            if end_date:
                assignments_recurrence = (
                    self._materialize_recurrence_assignments(
                        recurrence=recurrence,
                        start_date=recurrence.start_date,
                        end_date=end_date,
                        schedule_id=None,  # Schedule-independent
    ...
    def _materialize_recurrence_assignments(
        ...
        if check_existing and dates_recurring:
            ...
            dates_recurring = [
                d for d in dates_recurring if d not in existing_dates
            ]
    ...
    def get_assignments_and_recurrences(
        ...
        if not include_campaign:
            schedules = self.collection.schedule_db.get_schedules(
                team_id=team_id
            )
            ...
            assignments = [
                ...
                if a.schedule_id not in campaign_schedule_ids
            ]
        ...
        if shift_types is not None:
            ...
            shift_type_map: Dict[str, ShiftType] = {
                s.id: s.shift_type for s in shifts
            }
    ...
    def _lazy_materialize_recurrences(
        ...
        for recurrence in recurrences:
            ...
            materialization_start = max(
                watermark + timedelta(days=1), start_date
            )
    ...
    def _handle_recurrence_update(
        ...
        if recurrence_update_scope == RecurrenceUpdateScope.FUTURE:
            ...
            if (
                ...
                or recurrence_old.last_materialized_until
                > recurrence_old.end_date
            ):
                recurrence_old.last_materialized_until = (
                    recurrence_old.end_date
                )
            recurrence_updated = (
    ...
    def bulk_create_assignments(
        ...
        assignments_to_create: List[Assignment] = [
            Assignment(
                ...
                team_id=team_id,
                schedule_id=None,
                worker_id=entity_id if group_by == "shift" else cell.row_id,
                shift_id=entity_id if group_by == "worker" else cell.row_id,
    ...
    def bulk_update_assignments(
        ...
        The caller supplies the explicit change via *new_worker_id* /
        *new_shift_id* (exactly one is non-None, depending on the groupBy axis).
        The backend fetches existing assignments by ID and applies the change,
        so no client-supplied assignment objects are accepted.

        When *intent* is provided the backend also resolves the implicit
        campaign-scope selection server-side and applies the same change to
        those assignments.
        """
        out = AssignmentsRecurrencesResult(
            assignments_created=[],
        ...
        if intent:
            ...
            if intent_ids:
                for assignment_id in intent_ids:
                    ...
                    out.assignments_deleted_ids.extend(
                        ar.assignments_deleted_ids
                    )
                    out.assignments_created.extend(ar.assignments_created)
                    out.assignments_updated.extend(ar.assignments_updated)
    ...
    def _resolve_intent_assignments(
        ...
        return (
            self.collection.assignment_db.get_assignments_by_campaign_intent(
    ...
    def _apply_changes_to_database(
        ...
        a_created = self.collection.assignment_db.create_assignments(
            a_to_create
        )
