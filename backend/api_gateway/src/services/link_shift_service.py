from shared.schemas.core import LinkShift, LSChange, Shift
...
class LinkShiftService(BaseService):
    ...
    def update_link_shift_upon_shift_update(self, shift: Shift) -> LSChange:
    ...
    def update_link_shift_upon_shift_delete(self, shift: Shift) -> LSChange:
        ls_shift = self.collection.link_shift_db.get_link_shifts_by_shift_id(shift.id)
        shifts_ids = list(
            set(
                shift_id
        ...
        for ls in ls_shift:
            ...
            valid = ls.validate(shifts_ls, ls_others)
            if valid.is_valid:
                out.updated.append(self.collection.link_shift_db.update_link_shift(ls))
            else:
                self.collection.link_shift_db.delete_link_shift(ls.id)
                out.deleted.append(ls.id)
        return out
