...
## Our Principles (in priority order)
...
### Rockilus App — Backend (API Gateway)
- **Language:** Python
...
### Rockilus App — Backend (Schedule Generation Service)
- **Language:** Python
...
### Rockilus App — Backend (Shared Library)
- **Language:** Python
...
- Use `loguru` for all structured logging — never `print()` or stdlib `logging`.
...
**Rockilus Web:**
```bash
cd rockilus-web && npm install && npm run dev
```
...
| `frontend/src/app/i18n/locales/` | i18n translation files (en, fr, es) |
...
1. `backend/shared/src/schemas` — list DTOs and confirm canonical types
